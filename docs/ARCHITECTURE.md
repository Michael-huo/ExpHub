...
1. **`segment` (切片锚定)**：读取原始数据集，按时间戳抽取关键帧，构建时间网格。`scripts/segment_make.py` 保持稳定入口，内部实现下沉到 `scripts/_segment/`，以便后续扩展不同分段策略而不影响上游调度。
...
- `segment/`：存放抽取后的标准帧序列与元数据，核心产物包括 `frames/`、`keyframes/`、`timestamps.txt`、`calib.txt`、`preprocess_meta.json`、`step_meta.json`。
