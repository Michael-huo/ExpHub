...
- **极致解耦 (Platform Decoupling)**：
  ...
  - **统一 phase 配置**：各解释器统一定义在 `config/platform.yaml -> environments.phases.<phase>.python`，例如 `segment / prompt / prompt_smol / infer / slam`。
  - **配置形态**：
    ```yaml
    environments:
      phases:
...
- **检查范围**：doctor 默认读取 core phase 的解释器配置，即 `segment / prompt / infer / slam / prompt_smol`；若显式切回 `--prompt_backend qwen`，核心 `prompt` phase 仍必须可用。
