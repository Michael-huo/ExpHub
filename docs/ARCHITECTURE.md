...
2. **`prompt` (提示词生成)**：由 `prompt_gen.py` 前端完成 clip 采样与产物写回，再按 backend 路由到 VLM（当前支持 Qwen2-VL 与 SmolVLM2）提取结构化 `intent_card`，并在落盘前编译回 infer 可直接消费的 legacy `delta_prompt / delta_neg_prompt`。
3. **`infer` (视频生成推理)**：由 `infer_i2v.py` 先完成 request 归一与 schedule 解析，再按 infer backend 调用 I2V 模型 (如 VideoX-Fun/Wan) 进行两帧之间的插帧推理；当 manifest 满足 `prompt_manifest_v2` 时，infer 还会通过独立 consumer 显式消费 `global_invariants / intent_card / control_hints`，为每段生成 prompt 与 runtime override。
...
- `execution manifest`：复用 `prompt/manifest.json` 中的 `segments[*]`，由 deploy schedule 派生并被 `prompt / infer / merge` 直接消费；当前 manifest 已升级到 `prompt_manifest_v2`，infer v1 会优先消费结构化字段，但仍保留 legacy delta 字段兼容旧链路。
...
- `scripts/prompt_gen.py` 只负责 CLI 参数归一、按 `sample_mode` 选图、调用 backend、解析/清洗结构化 intent、再写回 `segment/clip_prompts.json` 与 `prompt/manifest.json`。
...
- `scripts/infer_i2v.py` 现在只负责读取 `segment/frames`、`prompt/manifest.json`、`deploy_schedule.json`，并构造统一 infer request；真正的 manifest v2 prompt/policy 解析在 backend runtime 内完成。
...
- `scripts/_infer/manifest_v2_consumer.py` 提供 infer 侧的薄策略层：识别 manifest v2、重编译 segment prompt，并把 `motion_intensity / geometry_priority / risk_level` 映射为保守的 runtime overrides。
- `scripts/_infer/backends/wan_fun_runtime.py` 现承载 Wan-Fun 14B/5B 共用 runtime：包含 profile 解析、pipeline 构造骨架、同段双卡协同 worker、segment-level prompt/policy override 执行、统一保存逻辑与分布式清理。
