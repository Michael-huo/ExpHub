...
| `scripts/prompt_gen.py` | `prompt` | `prompt` 前端入口；负责抽取 3 到 5 张代表帧、调用 backend 做闭集 `PromptProfile` 分类、多帧投票聚合，并写回 `profile.json / final_prompt.json / step_meta.json` |
| `scripts/_prompt/` | `prompt` (内部实现) | 内聚 prompt backend 抽象、采样策略、`PromptProfile` 闭集字段/模板/聚合器，以及具体 VLM 实现；当前包含 `qwen` 与 `smolvlm2` 两个 backend |
| `scripts/infer_i2v.py` | `infer` (外壳) | infer 前端入口；负责读取 frames / schedule / `prompt/final_prompt.json`，构造统一 request，按 backend 路由并写回 `runs_plan.json / step_meta.json` |
| `scripts/_infer/` | `infer` (内部实现) | infer backend 抽象层；当前包含最小 prompt file consumer、`wan_fun_runtime.py` 公共 runtime、`wan_fun_a14b_inp` / `wan_fun_5b_inp` 两个平级 backend，以及统一 request / factory |
...
| `config/prompt_manifest.json`| 历史配置文件 | 旧 `prompt_manifest_v2` 模板库；当前主链路已不再引用 |
