...
- `infer/policy_debug.json`
  - 保存完整的 consumer 输出与默认 profile 值，便于对照策略来源
- `infer/runs/*/params.json`
  - 保存该段最终执行时真正使用的 prompt、neg prompt、steps、guidance 和 source
- `infer/step_meta.json`
  - 汇总 `manifest_consumer_mode / prompt_source_counts / policy_source_counts`
