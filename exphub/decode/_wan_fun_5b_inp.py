...
def load_image_gen_runtime(path, default_prompt="", default_negative_prompt=""):
    ...
    if not isinstance(payload, dict):
        raise ValueError("decode task runtime must be a JSON object: {}".format(path))
    ...
    source = _collapse_ws(payload.get("source", "")) or "decode.native_tasks"
    ...
    for idx, raw_item in enumerate(list(payload.get("segments") or [])):
        ...
        start_idx = int(item.get("aligned_start_idx", item.get("start_idx")))
        end_idx = int(item.get("aligned_end_idx", item.get("end_idx")))
        num_frames = int(item.get("aligned_num_frames", item.get("num_frames", end_idx - start_idx + 1)))
        if end_idx < start_idx or num_frames != end_idx - start_idx + 1:
            raise ValueError("invalid decode task segment range at index {}".format(idx))
        prompt = _collapse_ws(item.get("resolved_prompt", item.get("prompt", ""))) or base_prompt
        if not prompt:
            raise ValueError("decode task segment {} missing prompt".format(idx))
        segments.append({
            "seg": int(item.get("seg", idx)),
            "segment_id": int(item.get("segment_id", idx)),
            "schedule_source": _collapse_ws(item.get("schedule_source", "")) or "decode.native_tasks",
            "execution_backend": _collapse_ws(item.get("execution_backend", "")) or execution_backend,
            "run_id": str(item.get("run_id", "run_{:03d}".format(idx)) or "run_{:03d}".format(idx)),
            "run_name": str(item.get("run_name", item.get("unit_id", "")) or "unit_{:04d}".format(idx)),
            "source_unit_id": str(item.get("source_unit_id", item.get("unit_id", "")) or ""),
            "source_span_id": str(item.get("source_span_id", "") or ""),
            "source_prompt_ref": dict(_as_dict(item.get("source_prompt_ref"))),
            "target_num_frames": int(item.get("target_num_frames", num_frames) or num_frames),
            "start_idx": int(start_idx),
            "end_idx": int(end_idx),
            "raw_start_idx": int(item.get("raw_start_idx", start_idx)),
            "raw_end_idx": int(item.get("raw_end_idx", end_idx)),
            "desired_start_idx": int(item.get("desired_start_idx", start_idx)),
            "desired_end_idx": int(item.get("desired_end_idx", end_idx)),
            "desired_num_frames": int(item.get("desired_num_frames", num_frames)),
            ...
            "negative_prompt": str(item.get("negative_prompt", negative_prompt) or negative_prompt),
...
def resolve_image_gen_runtime_segments(
    ...
    runtime_segments = list(runtime_payload.get("segments") or [])
    resolved = []
    for seg in range(max(0, int(num_segments))):
        item = _as_dict(runtime_segments[seg] if seg < len(runtime_segments) else {})
        steps = item.get("num_inference_steps", default_num_inference_steps)
        guidance = item.get("guidance_scale", default_guidance_scale)
        if steps in (None, ""):
        ...
        resolved.append({
            ...
            "resolved_prompt": _collapse_ws(item.get("resolved_prompt", "")) or _collapse_ws(default_prompt),
            "negative_prompt": _collapse_ws(item.get("negative_prompt", "")) or _collapse_ws(default_negative_prompt),
            "prompt_source": _collapse_ws(item.get("prompt_source", "")) or str(runtime_payload.get("source", "decode.native_tasks")),
            "num_inference_steps": int(steps),
            "guidance_scale": float(guidance),
            "aligned_start_idx": item.get("aligned_start_idx"),
            "aligned_end_idx": item.get("aligned_end_idx"),
            "aligned_num_frames": item.get("aligned_num_frames"),
        })
    return resolved
...
def run_wan_fun_backend_cli(argv=None, backend_config=None):
            ...
            segments.append(
                {
                    ...
                    "desired_start_idx": int(item.get("desired_start_idx", start_idx)),
                    "desired_end_idx": int(item.get("desired_end_idx", end_idx)),
                    "desired_num_frames": int(item.get("desired_num_frames", item.get("num_frames", end_idx - start_idx + 1))),
                    "aligned_start_idx": int(item.get("aligned_start_idx", start_idx)),
                    "aligned_end_idx": int(item.get("aligned_end_idx", end_idx)),
                    "aligned_num_frames": int(item.get("aligned_num_frames", item.get("num_frames", end_idx - start_idx + 1))),
                    "deploy_start_idx": int(item.get("deploy_start_idx", start_idx)),
                    "deploy_end_idx": int(item.get("deploy_end_idx", end_idx)),
                    "start_idx": int(start_idx),
                    "end_idx": int(end_idx),
    ...
    def run_one_segment(
        ...
        # type: (str, str, int, str, str, str, int, float, Optional[int], str) -> SegmentRunResult
    ...
    def save_results(result, save_context):
        ...
        run_name = str(result.run_name or "").strip()
        if not run_name:
    ...
    if args.batch:
        ...
        for seg, seg_spec in enumerate(batch_execution_segments):
            ...
            segment_result = run_one_segment(
                ...
                run_name=str(seg_spec.get("run_name", "") or ""),
            )
...
WAN_FUN_5B_BACKEND_CONFIG = WanFunRuntimeConfig(
    backend_name="wan_fun_5b_inp",
    config_name="wan_fun_5b_inp",
...
class WanFun5BInpBackend(WanFunInferBackend):
    ...
    default_phase = "infer_fun_5b"
    model_config_keys = ("wan2_2_fun_5b_inp",)
    backend_config = WAN_FUN_5B_BACKEND_CONFIG


def run_wan_fun_5b_backend_cli(argv=None):
    run_wan_fun_backend_cli(argv, backend_config=WAN_FUN_5B_BACKEND_CONFIG)
