...
from exphub.config import load_datasets_cfg, resolve_dataset
...
def _export_profile(args):
    return clips_build.build_export_profile(
...
def _run_encode_for_clip(runtime, target, export_root, clip_plan, profile):
    from exphub.runner import build_runtime
...
def _metadata_entry(export_root, split, clip_path, caption):
    return {
        "file_path": dataset_write.relative_to_root(export_root, clip_path),
        "text": str(caption),
        "type": "video",
        "split": str(split),
    }
...
def run(runtime):
    ...
    layout = dataset_write.ensure_layout(export_root)
        ...
        for clip_plan in harvest_plan_items:
            ...
            try:
                ...
                for candidate in candidates:
                    ...
                    split = dataset_write.assign_split(clip_key, seed=int(args.export_split_seed))
                    clip_filename = clips_build.build_clip_filename(
                        target["dataset"],
                        target["sequence"],
                    ...
                    clip_manifest_path = dataset_write.write_clip_manifest(
                        export_root,
                    ...
                    clip_record = {
                        ...
                        "train_clip_manifest": dataset_write.relative_to_root(export_root, clip_manifest_path),
    ...
    metadata_paths = dataset_write.write_metadata_files(export_root, entries_by_split)
    dataset_report = report_build.build_dataset_report(
        export_root=export_root,
        scope=str(args.export_scope),
        focus_name=str(_focus_name(args)),
        training_spec=clips_build.training_spec(profile),
        targets=targets,
...
def _build_arg_parser():
    ...
    parser.add_argument("--export_target_fps", type=int, default=clips_build.DEFAULT_EXPORT_FPS)
    parser.add_argument("--export_target_num_frames", type=int, default=clips_build.DEFAULT_EXPORT_NUM_FRAMES)
    parser.add_argument("--export_target_width", type=int, default=clips_build.DEFAULT_EXPORT_WIDTH)
    parser.add_argument("--export_target_height", type=int, default=clips_build.DEFAULT_EXPORT_HEIGHT)
