...
from .meta import auto_kf_gap, build_exp_name, sanitize_token, write_exp_meta
...
def write_compression_stats(exp_dir: Path) -> None:
    seg_dir = exp_dir / "segment"
    frames_dir = seg_dir / "frames"
    keyframes_dir = seg_dir / "keyframes"
    ...
    manifest = exp_dir / "prompt" / "manifest.json"
    ...
    (stats_dir / "compression.json").write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    kf_gap = auto_kf_gap(args.fps, args.kf_gap)
    if kf_gap % 4 != 0:
        _warn(f"kf_gap={kf_gap} not divisible by 4 (r=4). model may truncate length.")

    exp_name = build_exp_name(tag, args.w, args.h, args.start_sec, args.dur, args.fps, kf_gap)

    exp_root = Path(args.exp_root).resolve() if args.exp_root else (exphub_root / "experiments" / dataset / sequence)
    exp_dir = (exp_root / exp_name).resolve()
    segment_dir = exp_dir / "segment"
    prompt_dir = exp_dir / "prompt"
    infer_dir = exp_dir / "infer"
    merge_dir = exp_dir / "merge"
    slam_root = exp_dir / "slam"
    ...
    def write_meta_snapshot() -> None:
        ...
        write_exp_meta(exp_dir / "exp_meta.json", meta)
    ...
    def step_segment() -> None:
        ...
        _ensure(segment_dir / "calib.txt", "file")
        _ensure(segment_dir / "timestamps.txt", "file")
        # recommended keep
        _ensure(segment_dir / "preprocess_meta.json", "file")
    ...
    def step_prompt() -> None:
        ...
        cmd = [
            ...
            "--out_json",
            str(segment_dir / "clip_prompts.json"),
            "--out_manifest",
            str(prompt_dir / "manifest.json"),
        ]

        _conda_exec_logged(cmd, env_name=args.conda_env_vlm, log_name="prompt.log")
        _ensure(prompt_dir / "manifest.json", "file")
    ...
    def step_stats() -> None:
        ...
        _ensure(exp_dir / "stats" / "report.json", "file")
    ...
    def step_infer() -> None:
        ...
        _ensure(infer_dir / "runs", "dir")
        _ensure(infer_dir / "runs_plan.json", "file")
    ...
    def step_merge() -> None:
        ...
        cmd_merge = [
            ...
            str(infer_dir / "runs"),
            "--plan",
            str(infer_dir / "runs_plan.json"),
            "--out_dir",
            str(merge_dir),
        ]
        _conda_exec_logged(cmd_merge, env_name=args.conda_env_videox, log_name="merge.log")

        _ensure(merge_dir / "frames", "dir")
        _ensure(merge_dir / "calib.txt", "file")
        _ensure(merge_dir / "timestamps.txt", "file")
    ...
    def step_slam() -> None:
        ...
        def _run(tag_name: str, seg_path: Path) -> None:
            ...
            _ensure(dst_dir / "traj_est.tum", "file")
            _ensure(dst_dir / "run_meta.json", "file")

            # Ensure run_meta paths point to final track directory.
            try:
                meta_obj = json.loads((dst_dir / "run_meta.json").read_text(encoding="utf-8"))
            except Exception:
                meta_obj = {}
            ...
            tum_expect = (dst_dir / "traj_est.tum").resolve()
            npz_expect = (dst_dir / "traj_est.npz").resolve()
            ...
            _info(f"[OK] slam {tag_name} saved: {dst_dir / 'traj_est.tum'}")

        if seq in ("ori", "both"):
            _ensure(segment_dir / "frames", "dir")
        if seq in ("gen", "both"):
            _ensure(merge_dir / "frames", "dir")
    ...
    def step_eval() -> None:
        ...
        tum_ori = slam_root / "ori" / "traj_est.tum"
        tum_gen = slam_root / "gen" / "traj_est.tum"

        def _run_traj(name: str, tum: Path) -> None:
            _info(f"STEP eval: evo_traj {name} ({tum})")
            out_txt = eval_dir / ("evo_traj_" + name + ".txt")
            if viz_enable:
                out_png = eval_dir / ("traj_" + name + ".png")
        ...
        if tum_ori.exists() and tum_gen.exists() and _has_tool("evo_ape"):
            ...
            out_txt = eval_dir / "evo_ape_gen_vs_ori.txt"
            if viz_enable:
                out_png = eval_dir / "ape_gen_vs_ori.png"
    ...
    try:
        if args.mode == "segment":
            ...
            _run_step("prompt", step_prompt, str(prompt_dir / "manifest.json"))
        elif args.mode == "infer":
            _run_step("infer", step_infer, str(infer_dir / "runs_plan.json"))
        elif args.mode == "merge":
            _run_step("merge", step_merge, str(merge_dir))
        elif args.mode == "slam":
            _run_step("slam", step_slam, str(slam_root))
        elif args.mode == "eval":
            _run_step("eval", step_eval, str(exp_dir / "eval"))
        elif args.mode == "stats":
            _run_step("stats", step_stats, str(exp_dir / "stats" / "report.json"))
        else:  # all
            _run_step("segment", step_segment, str(segment_dir))
            _run_step("prompt", step_prompt, str(prompt_dir / "manifest.json"))
            _run_step("infer", step_infer, str(infer_dir / "runs_plan.json"))
