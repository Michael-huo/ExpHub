...
from _common import log_err, log_info, log_warn, write_json_atomic
...
def _append_warning(metrics_obj, msg):
    msg_text = str(msg)
    metrics_obj["warnings"].append(msg_text)
    log_warn(msg_text)
...
def _evaluate(args, metrics_obj):
    ...
    if metrics_obj["reference_pose_count"] <= 0:
        _append_warning(metrics_obj, "reference trajectory has no poses: {}".format(ref_path))
        return None
    if metrics_obj["estimate_pose_count"] <= 0:
        _append_warning(metrics_obj, "estimate trajectory has no poses: {}".format(est_path))
        return None
...
def _plot_traj_xy(plt, out_path, ref_traj, est_traj, ape_result, ref_label, est_label, metrics_obj):
    ...
    if est_xyz.shape[0] >= 2 and errors.size > 0:
        n = min(est_xyz.shape[0], errors.shape[0])
        est_xy = est_xyz[:n, :2]
        if est_xy.shape[0] >= 2:
...
def _generate_plots(args, plot_dir, eval_payload, metrics_obj):
    ...
    if ape_result is None or rpe_trans_result is None:
        _append_warning(metrics_obj, "skip plots because core metrics are unavailable")
        return
    ...
    if ref_traj is None or est_traj is None:
        _append_warning(metrics_obj, "aligned trajectories missing in APE result; skip traj plot")
    else:
        _plot_traj_xy(
            plt,
    ...
    if ape_curve is None:
        _append_warning(metrics_obj, "APE error curve unavailable")
    else:
        _plot_ape_curve(
            plt,
    ...
    if rpe_trans_curve is None:
        _append_warning(metrics_obj, "RPE translation curve unavailable")
    else:
...
def _fmt_value(value, unit):
    if value is None:
        return "n/a"
    return "{:.6f} {}".format(float(value), unit).strip()


def _summary_lines(metrics_obj, out_dir):
    lines = [
        "Eval Status      : {}".format(metrics_obj.get("eval_status")),
        "Reference        : {}".format(metrics_obj.get("reference_path")),
        "Estimate         : {}".format(metrics_obj.get("estimate_path")),
        "Alignment Mode   : {}".format(metrics_obj.get("alignment_mode")),
        "Reference Poses  : {}".format(metrics_obj.get("reference_pose_count")),
        "Estimate Poses   : {}".format(metrics_obj.get("estimate_pose_count")),
        "Matched Poses    : {}".format(metrics_obj.get("matched_pose_count")),
        "",
        "APE Translation",
        "  RMSE   : {}".format(_fmt_value(metrics_obj["ape_trans"].get("rmse"), "m")),
        "  Mean   : {}".format(_fmt_value(metrics_obj["ape_trans"].get("mean"), "m")),
        "  Median : {}".format(_fmt_value(metrics_obj["ape_trans"].get("median"), "m")),
        "  Std    : {}".format(_fmt_value(metrics_obj["ape_trans"].get("std"), "m")),
        "  Min    : {}".format(_fmt_value(metrics_obj["ape_trans"].get("min"), "m")),
        "  Max    : {}".format(_fmt_value(metrics_obj["ape_trans"].get("max"), "m")),
        "",
        "RPE Translation",
        "  RMSE   : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("rmse"), "m")),
        "  Mean   : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("mean"), "m")),
        "  Median : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("median"), "m")),
        "  Std    : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("std"), "m")),
        "  Min    : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("min"), "m")),
        "  Max    : {}".format(_fmt_value(metrics_obj["rpe_trans"].get("max"), "m")),
        "",
        "RPE Rotation",
        "  RMSE   : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("rmse"), "deg")),
        "  Mean   : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("mean"), "deg")),
        "  Median : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("median"), "deg")),
        "  Std    : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("std"), "deg")),
        "  Min    : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("min"), "deg")),
        "  Max    : {}".format(_fmt_value(metrics_obj["rpe_rot"].get("max"), "deg")),
        "",
        "Outputs",
        "  Metrics : {}".format(out_dir / "metrics.json"),
        "  Summary : {}".format(out_dir / "summary.txt"),
        "  Plots   : {}".format(out_dir / "plots"),
    ]
...
def _update_eval_status(metrics_obj):
...
def _write_outputs(out_dir, metrics_obj):
    summary_path = out_dir / "summary.txt"
    metrics_path = out_dir / "metrics.json"
    summary_text = "\n".join(_summary_lines(metrics_obj, out_dir)) + "\n"
    write_json_atomic(metrics_path, metrics_obj, indent=2)
    summary_path.write_text(summary_text, encoding="utf-8")


def _log_terminal_summary(metrics_obj, out_dir):
    ...
    prefix("eval metric: APE RMSE={}".format(_fmt_value(metrics_obj["ape_trans"].get("rmse"), "m")))
    prefix("eval metric: RPE trans RMSE={}".format(_fmt_value(metrics_obj["rpe_trans"].get("rmse"), "m")))
    prefix("eval metric: RPE rot RMSE={}".format(_fmt_value(metrics_obj["rpe_rot"].get("rmse"), "deg")))
...
def _parse_args():
    ...
    ap.add_argument("--reference_name", default="ori")
    ap.add_argument("--estimate_name", default="gen")
    ap.add_argument(
        "--alignment_mode",
        default="se3",
    ...
    ap.add_argument("--t_max_diff", type=float, default=0.01)
    ap.add_argument("--t_offset", type=float, default=0.0)
    ...
    try:
        ...
        if eval_payload is not None:
            try:
                ...
                _append_warning(metrics_obj, "failed to generate plots: {}".format(exc))
    except ImportError as exc:
        _append_warning(metrics_obj, "missing eval dependency: {}".format(exc))
    except Exception as exc:
        log_err("unexpected eval failure: {}".format(exc))
        _append_warning(metrics_obj, "unexpected eval failure: {}".format(exc))

    _update_eval_status(metrics_obj)
    _write_outputs(out_dir, metrics_obj)
    _log_terminal_summary(metrics_obj, out_dir)
