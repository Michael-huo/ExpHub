...
from scripts._common import list_frames_sorted, log_err, log_info, log_prog, log_warn, write_json_atomic
from scripts._schedule import build_wan_r4_deploy_schedule

from .api import materialize_keyframe_plan
from .policies.naming import (
...
def run_segment_make_cli(argv=None):
    ap = build_arg_parser()
            ...
            step_meta["outputs"]["keyframe_policy"].update(
                {
                    ...
                    "high_gap": int(state_meta.get("high_gap", 0) or 0),
...
def main(argv=None):
    return run_segment_make_cli(argv)
