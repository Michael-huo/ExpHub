...
def run_wan_fun_backend_cli(argv=None):
    # type: (object) -> None
    import warnings
    ...
    def _samefile_safe_path(src: Path, dst: Path) -> bool:
    ...
    if args.prompt_manifest.strip():
        ...
        import shutil
        dst = _manifest_dst.resolve()
        if _samefile_safe_path(src, dst):
            rprint(f"[INFO] prompt_manifest already at standard path, skip copy: {dst}")
        else:
            dst.parent.mkdir(parents=True, exist_ok=True)
            shutil.copy2(str(src), str(dst))
            rprint(f"[INFO] prompt_manifest copied to standard path: {src} -> {dst}")
    ...
    save_ctx = {
        ...
        "vae_temporal_compression_ratio": int(r),
    }
    ...
    def run_one_segment(
        ...
        seg_seed: int,
        segment_prompt: str,
        segment_negative_prompt: str,
        desired_num_frames: int = None,
    ) -> SegmentRunResult:
        ...
        with torch.no_grad():
            ...
            rprint(
                ...
                f"s={os.path.basename(start_path)} e={os.path.basename(end_path)} "
                f"seed={current_seed} ({current_video_length_run} frames, {sample_size[1]}×{sample_size[0]})..."
            )
            ...
            if _is_main_process():
                result_sample_tensor = _out.videos
            else:
    ...
    def save_results(result: SegmentRunResult, save_context):
        if result.result_sample is None:
            return

        os.makedirs(save_context["runs_root"], exist_ok=True)

        width, height = save_context["sample_size"][1], save_context["sample_size"][0]
        ...
        params = {
            ...
            "segment_seconds": (
                ...
                if save_context["is_batch"] else None
            ),
    ...
    if args.batch:
        ...
        for seg, seg_spec in enumerate(batch_execution_segments):
            ...
            if int(seg) < len(resolved_prompts):
                _rp = resolved_prompts[int(seg)]
                segment_prompt = _rp["final_prompt"]
                segment_negative_prompt = _rp["final_neg_prompt"]
                ...
                if segment_negative_prompt:
                    rprint(f"[PROMPT] seg {seg_i}/{total} neg={_escape_one_line(segment_negative_prompt)}")
            ...
            rprint(f"[INFO] seg {seg_i}/{total}: infer={segment_result.infer_sec:.2f}s save={t_save:.2f}s elapsed={elapsed:.1f}s eta={eta:.1f}s")
        ...
        if _rp is not None:
            ...
            if segment_negative_prompt:
                rprint(f"[PROMPT] single neg={_escape_one_line(segment_negative_prompt)}")
        segment_result = run_one_segment(
...
class WanFunA14BInpBackend(DirectInferBackend):
    ...
    def run(self, request):
        ...
        return super(WanFunA14BInpBackend, self).run(request)

    def _run_direct(self, argv):
        # type: (list) -> None
        run_wan_fun_backend_cli(argv)
